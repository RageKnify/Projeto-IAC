...
X_:     	MOV     R4,M[cruzes]    ;retira o numero de cruzes
        	CMP		R4,4			;ve se ha 4 cruzes (jogo acaba)
			BR.NZ	n_acaba
			MOV		R6,1
n_acaba:	MOV     R5,0
C_x:    	CMP     R5,R4
...
mais_nove:	PUSH	R5				;guardar R5(contador de ciclos_ROR), funçao estraga-o
			MOV		R5,M[codigo_m]	;mais_nove > poe na posiçao um algarismo incompativel
			ADD		R5,8h		;adiciona 8 para ter um algarismo maior que 6
			MOV		M[codigo_m],R5
			MOV		R5,M[tentativa_m]
			ADD		R5,9h		;adiciona 9 para ter um algarismo maior que 6(nao pode ser 8 porque seria 'x' e 'o'
			MOV		M[tentativa_m],R5
			POP		R5				;reavem o R5
			RET
